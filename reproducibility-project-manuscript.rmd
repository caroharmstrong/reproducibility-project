...
### Demographics
Participants will be asked to indicate their age and gender. The exact questions and additional details are shown in Appendix A. 
...
## Data analysis
Descriptive statistics will be computed for demographic variables, including mean and standard deviation for age and, for gender and race, counts and proportions.

Using the `psych` package, we will compute Pearson's *r* for the association between `neuroticism` and `loneliness` and test the hypothesis that the correlation between `neuroticism` and `loneliness` is significantly ($\alpha$ = .001) greater than *r* = .30 in this sample. 
