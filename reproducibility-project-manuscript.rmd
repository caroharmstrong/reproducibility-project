...
data_simulated = data_simulated |> 
  mutate(neur_sum_sim = neur_1_sim + neur_2_sim + neur_3_sim + neur_4_sim + neur_5_sim + neur_6_sim + neur_7_sim + neur_8_sim,
         lic_avg_sim = (lic_1_sim + lic_2_sim + lic_3_sim + lic_4_sim + lic_5_sim + lic_6_sim + lic_7_sim + lic_8_sim + lic_9_sim + lic_10_sim)/10)
           #mean(c(lic_1_sim, lic_2_sim, lic_3_sim, lic_4_sim, lic_5_sim, lic_6_sim, lic_7_sim, lic_8_sim, lic_9_sim, lic_10_sim)))
```
...
```{r}
# Create tibble with frequencies for gender
data_simulated |>                           
  group_by(gender_sim) |> 
  summarize(n = n()) |> 
  mutate(freq = n / sum(n)) 
...
```{r}
# Create tibble with frequencies for racial and ethnic identities endorsed
data_simulated |>                           
  group_by(race_ethn_sim) |> 
  summarize(n = n()) |> 
  mutate(freq = n / sum(n)) 
```
...
ggplot(data = data_simulated, aes(x = neur_sum_sim, y = lic_avg_sim)) +
  ...
  xlab("Neuroticism") +
  ylab("Loneliness") + 
  labs(caption = "Figure 1. Loneliness vs. Neuroticism") +
